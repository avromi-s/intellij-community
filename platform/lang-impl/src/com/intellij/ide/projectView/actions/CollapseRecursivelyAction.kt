...
class CollapseRecursivelyAction : DumbAwareAction(), CustomComponentAction, ActionRemoteBehaviorSpecification.Frontend {
  ...
  override fun actionPerformed(e: AnActionEvent) {
    ...
    TreeUtil.collapseAll(c, c.visibleRowCount);
    // todo as:as needs to be asynchronous and use method like below
    //  collapseAll isn't good because it gets all others on the same level. we need only that folder and it's descendants
    TreeUtil.promiseCollapse(c, Int.MAX_VALUE) { path ->
      selection.any { selectedPath ->
        ...
        when {
          ...
          selectedPath.isDescendant(path) -> path.isIncludedInCollapseAll
    ...
    System.out.println("right b4 exiting actionPerformed");
  }

  override fun update(e: AnActionEvent) {
    val c = e.dataContext.getData(CONTEXT_COMPONENT)
    e.presentation.isEnabled = c is JTree && c.selectionCount > 0
    e.presentation.isVisible = Registry.`is`("ide.project.view.replace.collapse.all.with.collapse.recursively", true)
  }
  ...
  override fun createCustomComponent(presentation: Presentation, place: String): JComponent =
    object : ActionButton(this, presentation, place, ActionToolbar.DEFAULT_MINIMUM_BUTTON_SIZE) {
      override fun updateToolTipText() {
        ...
        val collapseAllShortcut = ActionManager.getInstance().getKeyboardShortcut("ProjectViewCollapseAll")
        // todo as:as likely need to add all the ProjectViewCollapseAll equivalents just as ProjectViewExpandAll has
        if (collapseAllShortcut != null) {
          tooltip.setDescription(StringUtil.escapeXmlEntities(ActionsBundle.message(
            "action.CollapseRecursively.shortcutHint",  // todo as:as this may need to be added as well, not sure
            KeymapUtil.getShortcutText(collapseAllShortcut)
          )))
        }
...
private val TreePath.isIncludedInCollapseAll: Boolean
  get() {
